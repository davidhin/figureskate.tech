...
def parse_upper_rect(page, rect):
    # h_lines = [ rect["bottom"] - 20, rect["bottom"] ] TODO OLD LINE
    h_lines = [ rect["top"], rect["bottom"] ]
    # v_lines = [ rect["x0"], 65, 260, 300, 340, 380, 420, 500, rect["x1"] ] TODO OLD LINE
...
def parse_elements(page, rect):
    ...
    cropped = page.within_bbox(rect)
    words = cropped.extract_words()
    h_start = [ w for w in words if (w["text"] == "Elements" or w["text"] == "EExleemcuetnetds") ][0]["bottom"] + 1 #TODO BAD HACK ON EXECUTED ELEMENTS JUMBLED READ
...
def parse_program_components(page, rect):
    ...
    for i in range(9):
        colname = "J{}".format(i + 1)
        for jscore in range(df[colname].size):
            if (isinstance(df[colname][jscore], str)):
                df[colname][jscore] = df[colname][jscore].replace(',','.')
        df[colname] = df[colname].astype(float)
